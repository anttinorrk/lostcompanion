...
import gamefiles.lostcompanion._



object LostCompanionTextUI extends App {
  ...
  private def run() = {
    ...
    while (!this.game.isOver) {
      if (this.game.lastVerb == "go" || this.game.lastVerb == "" || this.game.lastVerb == "help") {
        this.printAreaInfo()
      }
      this.playTurn()
    }
    println("\n" + this.game.goodbyeMessage)
    println(this.game.turnMessage)
  }
